#!/usr/bin/env bash
# Negative/control ablation: select high eta_h heads under reduced warmup.
set -euo pipefail
...
STATS_OUT="${STATS_OUT:-results/cfs/cfs_high_eta_${ENV_NAME}_seed${SEED}.csv}"
...
python finetune.py \
  ...
  --project cfs-d \
  --group "${ENV_NAME}" \
  --exp_name cfs_high_eta_reduced_seed${SEED} \
  "$@"
