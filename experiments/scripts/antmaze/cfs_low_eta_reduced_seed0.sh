#!/usr/bin/env bash
# Main CFS-D intervention: select low eta_h heads under reduced warmup.
set -euo pipefail
...
STATS_OUT="${STATS_OUT:-results/cfs/cfs_low_eta_${ENV_NAME}_seed${SEED}.csv}"
...
python finetune.py \
  ...
  --project cfs-d \
  --group "${ENV_NAME}" \
  --exp_name cfs_low_eta_reduced_seed${SEED} \
  "$@"
